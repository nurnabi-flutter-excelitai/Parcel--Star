

import 'package:flutter/material.dart';
 class AppColor{

   static const Color appbarTextColor = Color(0XFF0B4461);
   static const Color appbarIconColor = Color(0XFF0B4461);
   static const Color appButtonColor = Color(0XFF0B4461);
   static const Color appBackgroundColor = Color(0xFFB4B4B4);
   static const Color appContainerColor = Color(0xFFFFFFFF);
   static const Color appSubTextColor = Color(0xFF454545);
   static const Color appBalanceColor = Color(0xFF33CC66);
   static const Color appProfileIdColor = Color(0xFFFF7676);

   // barchart page color collection
   static const Color appChartBackgroundColor = Color(0xFFE7F5FF);
   static const Color barChartDelSucForgroundColor = Color(0xFF17C13C);
   static const Color barCharTotalPendinForgroundColor = Color(0xFFECB409);
   static const Color barCharAsinPercelForgroundColor = Color(0xFF0D99FF);
   static const Color barChartCancelParcelForgroundColor = Color(0xFFED3E3E);
   static const Color appPrimaryColor = Color(0XFF0B4461);
   static const Color parcelStatementPageTextColor = Color(0XFF666666);



 // statement screen color....

   static const Color statementScreenTextColor = Color(0XFF0B4461);

 }